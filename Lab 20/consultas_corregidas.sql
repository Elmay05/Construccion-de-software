...
SQL
select clave,rfc,fecha from entregan	✔️
...
where materiales.clave = entregan.clave	✔️
...
select * 
from entregan 
where clave=1450 AND clave IN
(select clave from entregan)✔️
...
select clave from entregan where numero IN (select numero from entregan where numero=5018)✔️
...
(select clave from entregan where clave=1000)	✔️
...
SQL
select * from entregan,materiales ✔️
...
SELECT * FROM materiales where descripcion LIKE 'Si%'	✔️

¿Qué resultado obtienes?
Explica que hace el símbolo '%'.
no importa que datos estén despues de el "si", puede o no haber mas caracteres
¿Qué sucede si la consulta fuera : LIKE 'Si' ?
Busca solo los que digan Si textualmente
¿Qué resultado obtienes?
nada, no tengo ninguna descripcion que sea Si, con el primero obtengo las sillas
...
SELECT CONCAT(descripcion, ', ', precio) AS Resumen FROM MateriaLes;	✔️
...
SELECT RFC FROM Entregan WHERE RFC REGEXP '^[A-D]';   RFC que inicien con A a D	✔️
SELECT RFC FROM Entregan WHERE RFC NOT LIKE 'A%';    RFC que no inicien con A	✔️
SELECT Numero FROM Entregan WHERE Numero LIKE '___6';	Numero que termine con 6 y tenga otros 3 caracteres antes	✔️
...
FROM Entregan
WHERE Numero Between 5000 and 5010;	✔️
STS Se utiliza para especificar dentro de una subconsulta la existencia de ciertas filas.
...
busca los numeros entre 5000 y 5010  y a demas lso RFC Se encuentran en la 2 consulta
¿Qué función tiene el paréntesis ( ) después de EXISTS?
consulta de los RFC de proveedores donde la razon social empieza con 'La'
...
SELECT * FROM Proyectos LIMIT 2;	✔️
Esta muestra todas las columnas de los primeros 2 datos de la tabla proyectos

¿Qué sucede con la siguiente consulta? Explica por qué.

SELECT Numero FROM Proyectos LIMIT 1;	✔️
Muestra el numero del primer proyecto que hay registrado
...
ALTER TABLE materiales ADD PorcentajeImpuesto DECIMAL(6, 2); ✔️
...
UPDATE materiales SET PorcentajeImpuesto = 2*clave/1000;	✔️
